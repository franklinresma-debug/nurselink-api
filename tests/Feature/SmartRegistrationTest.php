...
class SmartRegistrationTest extends TestCase
{
    ...
    public function test_member_confirmation_does_not_equal_officer_verification(): void
    {
        ...
        app(FactReviewService::class)->confirm($fact, $application, $user);
        $fact->refresh();
        $this->assertSame('confirmed', $fact->member_status);
        $this->assertSame('unverified', $fact->verification_status);
    }

    public function test_missing_field_service_identifies_required_fields(): void
    {
        [$user,$application] = $this->applicantWithApplication();
        $missing = app(MissingFieldService::class)->refresh($application);
        $this->assertGreaterThan(0, count($missing));
        $this->assertDatabaseHas('application_data_reviews', ['application_id' => $application->id, 'state' => 'missing']);
    }
    ...
    private function applicantWithApplication(): array
    {
        ...
        $user->roles()->attach(Role::query()->where('code', 'applicant')->firstOrFail());
