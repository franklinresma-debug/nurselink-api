...
class PreferenceService {
 public function get(User $user):NotificationPreference
 {
  $defaults=collect(config('communications.default_channels',[]))
   ->mapWithKeys(fn($enabled,$channel)=>[$channel.'_enabled'=>(bool)$enabled])
   ->all();
  return NotificationPreference::query()->firstOrCreate(
   ['user_id'=>$user->id],
   array_merge($defaults,['timezone'=>'Asia/Manila'])
  );
 }
